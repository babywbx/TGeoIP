...
- **⚡️ 高效并发**: 高并发检测，基于 `net/netip` 的内存高效 IP 处理、预解析排序和流式文件写入。
- **🪶 轻量依赖**: 仅一个外部依赖 (`maxminddb-golang`)，核心 IP 处理逻辑完全基于 Go 标准库。
- **🛡️ 可靠性高**: 默认使用 TCP 443 端口检测并配备 HTTP 超时，在云环境中比 ICMP ping 更可靠。
- **🌍 地理位置查询**: 使用本地 MMDB 数据库，查询速度快且支持离线。
- **📝 双格式输出**: 同时生成纯 IP 列表 (`US.txt`) 和聚合后的 CIDR 列表 (`US-CIDR.txt`)。
- **🔄 重试机制**: 实现 3 次重试逻辑，每次间隔 200ms，提高可靠性。
- **⏱️ 优化超时**: 检测使用 3 秒超时，HTTP 请求使用 30 秒超时。
...
5.  `wbxBot` 自动将更新后的文件提交到 `geoip` 分支。
